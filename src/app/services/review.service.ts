...
export class ReviewService {
  private baseUrl = 'http://localhost:3000/api/v1/review'; // غيريها لو عندك API مختلف
...
getBookReviews(slug: string): Observable<any> {
  ...
  submitReview(slug: string, data: any): Observable<any> {
    return this.http.post(`${this.baseUrl}/${slug}`, data, this.getAuthHeader());
  }

  updateReview(slug: string, data: any): Observable<any> {
    return this.http.patch(`${this.baseUrl}/${slug}`, data, this.getAuthHeader());
  }
